...
function transform_hast({
	...
	return function transformer(tree, vFile) {
		...
		visit$6(tree, 'element', (node) => {
			...
			// a11y: auto alt for images
			if (node.tagName === 'img') {
				if (!node.properties || node.properties.alt == null) {
					if (!node.properties) node.properties = {};
					const src = node.properties.src || '';
					const match = src.match(/\/([^\/]+\.\w+)$/);
					node.properties.alt = match ? match[1].replace(/\.\w+$/, '') : '图片';
				}
			}

			// a11y: aria-label for empty external links
			if (
				node.tagName === 'a' &&
				node.properties &&
				typeof node.properties.href === 'string' &&
...
const code_highlight = async (
	...
	//@ts-ignore
	if (!Prism) Prism = await import('./prism-Cz3w-km1.js').then(function (n) { return n.p; });
...
//# sourceMappingURL=main-DLA3kuAq.js.map
