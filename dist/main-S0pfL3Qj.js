...
function requireElement () {
	...
	const repeat = requireRepeatString();
...
var one = requireOne();
const repeat$1 = requireRepeatString();
...
const RE_MODULE_SCRIPT = new RegExp(
	`^(<script(?:\\s(?:module|context="module"))` + attrs + `>)`
);
	...
	return function transformer(tree, vFile) {
		...
		visit$6(tree, 'element', (node) => {
			if (
				...
				// a11y: add aria-label for external links with no text
				if (
			...
			if (
				...
				// a11y: auto alt for images without alt
				if (!node.properties.alt && node.properties.src) {
					const match = node.properties.src.match(/\/([^\/]+\.\w+)$/);
					node.properties.alt = match
						? match[1].replace(/\.\w+$/, '')
						: '图片';
				}
			}
		...
		visit$6(tree, 'root', (node) => {
			...
			// Transform component tags BEFORE extract_parts to preserve modifications
			if (components) {
				for (let i = 0; i < components.length; i++) {
					...
					visit$6(tree, 'raw', (node) => {
						const rawNode = node ;
						if (rawNode.value) {
							rawNode.value = rawNode.value.replace(
								new RegExp(`<${components[i]}([\\s][^>]*?)[\\s]*\\/?>`, 'g'),
								(_match, attrs) =>
									`<Components.${components[i]}${attrs} />`
							);
						}
					});
				}
...
const code_highlight = async (
	...
	//@ts-ignore
	if (!Prism) Prism = await import('./prism-BuDarg1w.js').then(function (n) { return n.p; });
...
//# sourceMappingURL=main-S0pfL3Qj.js.map
